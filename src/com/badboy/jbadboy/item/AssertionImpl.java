...
public class AssertionImpl extends Assertion {
    ...
    public void play() throws ExecutionException {
        for(ScriptItem c : getSubItems()) {
            if(c instanceof Check) {
                Check check = (Check) c;
                CheckResult result = check.check(this);
                if(result.status != CheckExecutionStatus.SUCCEEDED) {
                    this.addChild(new AssertionFailureImpl(result.getDescription()));
                    break;
                }
            }
        }
